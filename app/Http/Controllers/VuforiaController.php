...
class VuforiaController extends Controller
{
    ...
    const ACCESS_KEY = '5645b3a31e97026998e6063981bf4dd8ce824ca5';
    const SECRET_KEY = '500968f91a647acb87d804b3fcc3455bb75829e6';
    public $imagePath = '';
    public $imageName = '';

    public function addGet()
    {
        return view('targets/add');
    }
    ...
    public function index()
    {
        ...
        foreach ($rows->results as $row){
            $r = $this->getTargetRecord($row);
            array_push($targets,$r->target_record);
        }
    ...
    public function addTarget(Request $request) {
        ...
        if ($info['http_code'] !== 201) {
            ...
            return redirect()->route('index_form');
        }
    }
    ...
    public function deleteAllTargets() {
        ...
        foreach ($targets->results as $index => $id) {
            ...
            //return json_decode($response);
            Session::flash('message', 'Deleted All Targets Successfully');
            Session::flash('alert-class', 'alert-success');
